...
# TODO
- check covariance matrices, why so different?
- include figure from batch method
...
For the following boxplots, we used the batch method of 32 repetitions.

# Problem 5

The contents of `spam_snow.sh` are as follows. We check the number of arguments within the bash script and then check the cores argument within `spam_snow.R`. For `spam_snow.R`, we use `clusterApply()` with the class version of `spam_mc.R` as a function.
